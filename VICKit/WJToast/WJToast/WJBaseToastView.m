...
- (instancetype)initToastWithTitle:(NSString *)title message:(NSString *)message iconImage:(UIImage*)iconImage{
    ...
    if (iconImage == nil) {
        if (self.toastType == WJToastPositionDefaule) {
            self.iconImage = nil;
        }
    }else{
...
- (void)layoutToastView{
    ...
    switch (self.toastPosition) {
        ...
        case WJToastPositionBottomWithFillet: {
...
- (CGRect)toastViewFrame{
    ...
    if (self.toastPosition == WJToastPositionBelowStatusBarWithFillet) {
        textMaxWidth -= 2*HORIZONTAL_SPACE;
    }else if (self.toastPosition == WJToastPositionBottomWithFillet) {
...
-(void)dismiss{
    ...
    if (toastArray && [toastArray count] > 0) {
        @synchronized (toastArray) {
            ...
            WJBaseToastView* toast = toastArray[0];
            [NSRunLoop cancelPreviousPerformRequestsWithTarget:toast];
            [toastArray removeObject:toast];
            
            if (self.dismissToastAnimated == YES && _toastPosition != WJToastPositionBottomWithFillet) {
